...
$(document).ready(function(){
	// 옵션 select 를 선택했을 때 , change 이벤트 발생
	$('#goodsOptionId1').change(function(){
		...
		$.ajax({
			...
			type : "get",
			data : {
				// select 에서 선택한 값을 JSON 으로 조립해서 파라미터를 구성한다.
				goodsOptionName : $("#goodsOptionId1").val()
			},
			dataType : "html", // 콜백 데이터의 타입
			contentType : "text/html;charset:utf-8",
			success : function(d){
				// 1. hidden 으로 생성 된 totalPrice 의 값을 가져와서 숫자형으로 변경한다.
				var totalPrice = parseInt($('#totalPrice').val());
				// 2. 선택한 옵션의 value 을 가져온다.
