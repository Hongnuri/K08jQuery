...
public class NaverSearchAPI extends HttpServlet {
	...
	protected void service(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
			...
			String clientId = "FjAZKpMMIVM_FqvCsGnT"; // 애플리케이션 클라이언트 아이디값"
			String clientSecret = "KTPvbYtujy"; // 애플리케이션 클라이언트 시크릿값"

			String text = null;
			int startNum = 0;
			try {
				// JSP 에서 입력한 검색어를 파라미터로 받아서 변수 처리
			 ...
			 String apiURL = "https://openapi.naver.com/v1/search/blog?query=" + text+"&display=20&start="+startNum;    // json 결과
		        ...
		        // 네이버 검색 API 실행시의 결과값을 로그로 출력한다.
		        System.out.println(responseBody);
		        
		        // 네이버의 검색결과를 JSON 으로 내려 받아 서블릿에서 즉시 화면에 출력한다.
